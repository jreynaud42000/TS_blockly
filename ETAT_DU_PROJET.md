...
| Maqueen : sprite du robot | Coque CSS redessinée (bord doré, vis aux coins, bouton orange, DEL colorées) à la demande de l'utilisateur, inspirée de l'allure du vrai châssis sans image externe — cohérent avec le reste du simulateur (carte micro:bit, écran LCD, tous dessinés, jamais d'image). Les deux DEL du sprite vérifiées refléter la vraie couleur programmée (`simu_maqueenRgb`), pas seulement celles du panneau ; confirmé visuellement par capture à l'échelle ×4 |
